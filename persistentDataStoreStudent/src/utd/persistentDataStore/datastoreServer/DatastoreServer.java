...
public class DatastoreServer
{
	...
	private ServerCommand dispatchCommand(InputStream inputStream) throws ServerException, IOException
	{
		...
		if("write".equalsIgnoreCase(command)){
			ServerCommand serverCommand = new WriteHandler();
			return serverCommand;
		}
		else if("read".equalsIgnoreCase(command)){
			ServerCommand serverCommand = new ReadHandler();
			return serverCommand;
		}
		else if("delete".equalsIgnoreCase(command)){
			ServerCommand serverCommand = new DeleteHandler();
			return serverCommand;			
		}
		else if("directory".equalsIgnoreCase(command)){
			ServerCommand serverCommand = new DirectoryHandler();
			return serverCommand;
		}
		else{
			throw new ServerException("Unknown Request: " + command);
		}
